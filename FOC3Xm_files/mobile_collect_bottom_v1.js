...
if ($) {
    $(function() {
        ...
        var $preview_footer_btn = $preview_show.length > 0 ? $preview_show.find('#preview_footer_btn') : '';
...
function setPowerBy (name,son_name,obj,type1, ret, data_tips, code_cvid) {
    ...
    var ab_text =   '<a id="footer_powered_row_a" href="javascript:;" data-link="'+ ret.data.powered_link +'" target="_blank" data-params="25,'+ret.data.industry+'">'+
                        '<span style="margin-right:3px">该页面通过</span>草料二维码<span style="margin-left:3px">制作</span>' +
                    '</a>';
    ...
    if (ret.data.edition_id) {
        ...
        var content = content_tips + '<div id="footer_powered_content" class="clearfix">\
                                            <div id="footer_collect_row" class="footer_collect_row '+showCollect+'" style="margin-bottom: 8px">\
                                                <a id="footer_complain_row_a" href="//h5.clewm.net/complain?coding='+coding+'" class="footer_collect_row_a ' + hideComplain + '">投诉</a>\
                                                <span id="footer_divider" class="footer_divider ' + showDivider + '">|</span>\
                                                <a id="footer_collect_row_a" class="footer_collect_row_a ' + isCollectShow + '" href="//'+getDomain()+'/wx_authorization.html?coding='+coding+'&type=active" data-params="25,5">收藏</a>\
        ...
        doc.getElementById('footer_powered_row_a').onclick=function(e){
            ...
            if (is_weixin > -1) {
                if (window.__wxjs_environment === 'miniprogram') {
                    setTimeout(function() {
                        wx.miniProgram.navigateTo({url: '/pages/code/code?q=http://wxa.cli.im/qr/huoma/C_o2eikt/S_1/qgnpFUZ/21'})
                    }, 500)
                    return false;
                }
                setTimeout(function() {
